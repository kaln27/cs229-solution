...
def backward_softmax(x, grad_outputs):
    ...
    # *** START CODE HERE ***
    softmax = forward_softmax(x)
    jacobian = np.diag(softmax) - np.outer(softmax, softmax)
    return grad_outputs.dot(jacobian)
    # *** END CODE HERE ***
...
def backward_convolution(conv_W, conv_b, data, output_grad):
    ...
    # *** START CODE HERE ***
    out_c, inp_c, k_w, k_h = conv_W.shape
    
    grad_w = np.stack([
        forward_convolution(np.expand_dims(output_grad, 1), np.zeros(out_c), np.expand_dims(data[i], 0))
        for i in range(inp_c)
        ], axis = 1)
    grad_b = output_grad.sum(axis=(1, 2))
    rev_w = conv_W[:, :, ::-1, ::-1]
    rev_w = rev_w.transpose((1, 0, 2, 3))
    pad_grad = np.pad(output_grad, ((0, 0), (k_w-1, k_h-1), (k_w-1, k_h-1)))
    grad_data = forward_convolution(rev_w, np.zeros(inp_c), pad_grad)
...
def backward_max_pool(data, pool_width, pool_height, output_grad):
    ...
    inp_c, inp_w, inp_h = data.shape
    for c in range(inp_c):
        for i in range(pool_width):
            for j in range(pool_height):
                di, dj = np.unravel_index(np.argmax(data[c, i*pool_width : (i+1)*pool_width, 
                                          j*pool_height : (j+1)*pool_height]), (pool_width, pool_height))
                grad[c, i*pool_width+di, j*pool_height+dj] = output_grad[c, i, j]
...
def backward_prop(data, labels, params):
    ...
    flattened = np.reshape(first_after_relu, (-1))
    logits = forward_linear(W2, b2, flattened)
    prob = forward_softmax(logits)
    
    # backward
    grad_ce = backward_cross_entropy_loss(prob, labels)
    grad_sm = backward_softmax(logits, grad_ce)
    grad['W2'], grad['b2'], grad_linear = backward_linear(W2, b1, flattened, grad_sm)
    grad_flat = grad_linear.reshape(CONVOLUTION_FILTERS, MAX_POOL_SIZE, MAX_POOL_SIZE)
    grad_relu = backward_relu(first_max_pool, grad_flat)
    grad_mp = backward_max_pool(first_convolution, MAX_POOL_SIZE, MAX_POOL_SIZE, grad_relu)
    grad['W1'], grad['b1'], grad_conv = backward_convolution(W1, b1, data, grad_mp)
